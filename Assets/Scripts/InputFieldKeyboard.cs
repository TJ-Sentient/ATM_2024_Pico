...
public class InputFieldKeyboard : MonoBehaviour
{
    ...
    private void Update()
    {
        if (Input.GetKeyUp(KeyCode.Return) || Input.GetKeyUp(KeyCode.KeypadEnter)) closeKeyboard(); // If enter key pressed, close keyboard
    }
    ...
    private void closeKeyboard()
    {
        if (_keyboard != null)
        {
            try {
                ...
                UnityEngine.Debug.Log("Cannot kill keyboard process: " + e.Message);
            }
            _keyboard = null;
        }
    }
